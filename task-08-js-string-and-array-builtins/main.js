...
*******************************************************************************/
console.log( "-- Part 1, Task 1: ".padEnd( 79, "-" ) );

const words     = alice.replace( /\n/g, " " ).split( /\s+/ );
...
/* for ( let i of Array( 10 ).keys() ) { count.push( i + 1 ); } */
for ( let i = 1; i <= 10; i++ ) {
  count.push( i );
}
...
const findInContext = ( source, search, portion ) => {
  const index = source.indexOf( ` ${search} ` );
  const start = index - portion + 1;
  const end   = index + search.length + portion + 1;

  return source.slice( start, end );
};

const ifString  = findInContext( alice, "if",  5 );
const asString  = findInContext( alice, "as",  5 );
const andString = findInContext( alice, "and", 5 );
...
const catString = ifString.concat(asString).concat(andString);
...
let deathscenes = [
`the door is loose and fell on top of you`
];
...
while ( alive ) {
  ...
  if ( lives <= 0 ) {
    console.log( "You have no more lives, game over." );
    alive = false;
  }
}
...
for ( let i = 0; i < 10; i++ ) {
  randomWords.push( words[ getRandomInt( wordcount ) ] );
  randomNumbers.push( getRandomInt( wordcount ) );
}

console.log( `ten words:\n[ ${randomWords} ]` );
console.log( `\nten numbers:\n[ ${randomNumbers} ]` );
...
*  forbli uendret.                                                             *
*******************************************************************************/
console.log( "\n-- Part 4, Task 8: ".padEnd( 80, "-" ) );

const randomNumber = getRandomInt( wordcount );

/* randomNumbers.fill( randomNumber, 1, ( randomNumbers.length - 1 ) ) */
for ( let i = 1; i < ( randomNumbers.length - 1 ); i++ ) {
  randomNumbers.splice( i, 1, randomNumber );
}

console.log( `ten numbers:\n[ ${randomNumbers} ]` );
